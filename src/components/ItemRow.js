...
const ItemRow = (props) => {
  return (<div className={props.index % 2 === 0 ? `${ItemRowStyle.row} ${ItemRowStyle.color_bg}` : ItemRowStyle.row}>
    ...
    <div className={`${ItemRowStyle.col} ${ItemRowStyle.name}`} title={props.item.name}><a name={props.item._id} href={`#${props.item._id}`} style={{textDecoration: 'none'}}>■</a>&nbsp;{props.item.name}</div>
